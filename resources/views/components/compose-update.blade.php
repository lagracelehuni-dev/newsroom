    ...
    <div class="bloc__absolute">
        <div class="section section--compose section--scroll">
            <div class="section__header section__header--compose compose--header">
                <p class="section__title compose--title">Modifier l'article</p>
                <button class="section__header-close trigger-back"> <i class="ri ri-close-fill"></i> </button>
            </div>

            <div class="compose__body">
                @if(isset($post) && $post)
                    <form class="compose__form" method="POST" action="{{ route('posts.update', $post->id) }}" enctype="multipart/form-data">
                @else
